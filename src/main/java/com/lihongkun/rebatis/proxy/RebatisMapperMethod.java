...
public class RebatisMapperMethod {

	private final SqlCommand command;
	private final MethodSignature method;

	public RebatisMapperMethod(Class<?> mapperInterface, Method method, Configuration config) {
	...
	public Object execute(SqlSession sqlSession, Object[] args) {
		...
		if (SqlCommandType.INSERT == command.getType()) {
			...
			} else if(method.getReturnType().isAssignableFrom(Pagination.class)){
				// add by lihongkun 
				result = executeForPagination(sqlSession, args);
			}
			else {
				Object param = method.convertArgsToSqlCommandParam(args);
	 ...
	 * @return
	 */
	private <E> Object executeForPagination(SqlSession sqlSession, Object[] args) {
		
		//提取出分页参数
		Paginator paginator = PageHelper.getPaginator(args);
		...
		// if need to count total
		if(paginator.isContainTotal()){
			result.setTotal(PageHelper.getTotalCnt(sqlSession, command, param));
		}
		result.setItems(sqlSession.<E>selectList(command.getName(), param,new RebatisRowBounds(paginator.getOffset(), paginator.getLimit())));
